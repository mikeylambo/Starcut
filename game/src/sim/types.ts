import { clamp, wrapAngle } from "../core/DetMath";
...
/** Wire/replay form: [moveX*127, moveZ*127, yaw*8192, pitch*8192, packed counters]. */
...
export function packInput(i: SimInput): PackedInput {
  return [
    ...
    Math.round(wrapAngle(i.yaw) * LOOK_Q),
...
export function isValidPacked(p: unknown): p is PackedInput {
  ...
  return (
    ...
    Math.abs(p[2]) <= Math.ceil(Math.PI * LOOK_Q) + 1 &&
